...
robot_base::robot_base(){
	...
	******************************************/
	...
	motor1_L = new motor_driver(1,htim1,htim2,TIM_CHANNEL_1,TIM_CHANNEL_2,TIM_CHANNEL_1,0,900);
	motor2_L = new motor_driver(2,htim1,htim3,TIM_CHANNEL_3,TIM_CHANNEL_4,TIM_CHANNEL_1,0,900);
	motor1_R = new motor_driver(3,htim8,htim4,TIM_CHANNEL_1,TIM_CHANNEL_2,TIM_CHANNEL_1,0,900);
	motor2_R = new motor_driver(3,htim8,htim5,TIM_CHANNEL_3,TIM_CHANNEL_4,TIM_CHANNEL_4,0,900);

	robot_linear_vel_x = 0;
}
//robot_base类析构函数的实现
robot_base::~robot_base(){
	delete motor1_L;//将两个对象回收
	delete motor2_L;
	delete motor1_R;//将两个对象回收
	delete motor2_R;
}
...
void robot_base::velocity_to_RPM(float linear_vel_x,float angular_rad_z){
	...
	while(1){
		...
		motor_prm_1L = (linear_vel_x_min - tangential_vel ) / wheel_circumference;
		//这个是右轮
		motor_prm_2R = (linear_vel_x_min + tangential_vel ) / wheel_circumference;
		...
		if(abs(motor_prm_1L) <= motor_driver::_motor_max_rpm//因为可能是负值，所以要取一下绝对值，看他是否比最大转速要大
			&& abs(motor_prm_2R) <= motor_driver::_motor_max_rpm){
				break;
		}
...
void robot_base::run(){
	motor1_L->set_rpm(motor_prm_1L);//将算好了的电机转速通过参数传递
	motor2_L->set_rpm(motor_prm_2L);
	motor1_R->set_rpm(motor_prm_1R);//将算好了的电机转速通过参数传递
	motor2_R->set_rpm(motor_prm_2R);
	//然后将机器人的移动速度算出来，两个轮子的当前转速相加然后除以2就是真正机器人的速度了
	double average_rpm = (motor1_L->rpm_cur + motor1_L->rpm_cur)/2;
