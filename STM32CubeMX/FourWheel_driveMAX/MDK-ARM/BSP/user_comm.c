...
void user_comm_init(void){
	...
	do{
	}while(HAL_OK != HAL_UART_Receive_IT(&huart2,(uint8_t*)&serial_rx_data,1));//只进行一个字节一个字节读取，因为又同步码和结束码
...
void USART1_IRQHandler(UART_HandleTypeDef *huart){
	if(huart == &huart4){
		if(serial_rx_data.syn != _SERIAL_SYN_CODE_START){//判断是不是同步码，如果不是就再次接收一个，看是不是，如果还不是就结束程序，等待下一个接收的数据来，然后再判断
				do{
				}while(HAL_OK != HAL_UART_Receive_IT(&huart4,(uint8_t*)&serial_rx_data,1));//第二个参数是接收数据的缓冲区，第三个参数是接收数据的大小
				return ;
		}
		if(serial_rx_data.syn == _SERIAL_SYN_CODE_START && serial_rx_data.syn_CR !='\r' && serial_rx_data.syn_LF !='\n'){//如果在这个字节接收到了同步位，且还没有接收到数据位，那么这个字节也收了，然后程序结束
			do{
			}while(HAL_OK != HAL_UART_Receive_IT(&huart4,(uint8_t*)&serial_rx_data+1,sizeof(serialData)-1));//收一次，大小当然也要减少一个
			return ;
		}
		...
		serial_rx_data.syn = serial_rx_data.syn_CR = serial_rx_data.syn_LF = serial_rx_data.type = 0;
		do{
		}while(HAL_OK != HAL_UART_Receive_IT(&huart4,(uint8_t*)&serial_rx_data,sizeof(serialData)));
